...
import { useLayoutConfig, ModuleId, ModuleSizes } from "@/hooks/useLayoutConfig";
...
  // 'layout.moduleSizes' est désormais la source de vérité
...
      // Met à jour l'ordre, en conservant les tailles actuelles
      updateLayout(newOrder, layout.moduleSizes); 
...
  // ✅ CORRECTION: Met à jour la source de vérité globale (layout.moduleSizes)
  const handleModuleResize = (moduleId: string, width: number, height: number) => {
    const newModuleSizes: ModuleSizes = {
        ...layout.moduleSizes,
        [moduleId]: { width, height }
    };
    // Met à jour le layout entier (ordre + NOUVELLES tailles)
    updateLayout(layout.moduleOrder, newModuleSizes);
  };

  const handleSaveLayout = (name: string) => {
    // ✅ Utilise la source de vérité pour la sauvegarde
    saveLayout(name, layout.moduleOrder, layout.moduleSizes);
...
    if (savedLayout) {
      // ✅ Charge les tailles et l'ordre dans la source de vérité globale
      updateLayout(savedLayout.moduleOrder, savedLayout.moduleSizes);
      setCurrentLayoutName(name);
...
                // ✅ Lit la taille depuis la source de vérité globale
                const savedSize = layout.moduleSizes[moduleId];
...
                      id={moduleId}
                      // ✅ Passe la taille comme props 'width' et 'height'
                      width={savedSize?.width || 460} 
                      height={savedSize?.height || 345}
